<p align="center">
  <img src="./docs/images/readme-main.png" alt="Fragmnt 브랜딩 목업" width="100%" />
</p>
...
| 카테고리 | 기술 | 사용 목적 |
| --- | --- | --- |
| Core | React | 컴포넌트 기반 UI 구현 |
| Core | TypeScript | 정적 타입 기반의 컴포넌트 및 API 응답 처리 |
| Build | Vite | 빠른 개발 서버와 번들링 환경 구성 |
| Routing | TanStack Router | 라우트 기반 페이지 구조와 동적 라우팅 관리 |
| Server State | TanStack Query | 서버 상태 관리, API 요청 캐싱, 데이터 무효화 처리 |
| Client State | Zustand | 인증 정보 등 클라이언트 전역 상태 관리 |
| API | Axios | API 요청 인스턴스 및 인터셉터 기반 HTTP 통신 처리 |
| Styling | Tailwind CSS | 토큰 기반 반응형 UI 스타일링 |
| UI | shadcn/ui | 접근성과 재사용성을 고려한 UI 컴포넌트 구성 |
| UI | lucide-react | 일관된 아이콘 시스템 적용 |
| Animation | Framer Motion | 추천 플로우 내 전환 및 인터랙션 애니메이션 구현 |
| Asset | SVGR | SVG 에셋을 React 컴포넌트로 활용 |
| Quality | ESLint | 코드 품질 검사 |
| Quality | Prettier | 코드 포맷팅 |
| Workflow | husky | Git Hook 기반 커밋 전 자동화 |
| Workflow | lint-staged | 변경 파일 대상 린트 자동 실행 |
| Workflow | commitlint | 커밋 메시지 컨벤션 관리 |
